...
# Script d'automatisation de validation LibVisualMem v3.0
# =======================================================
# Automatise la création, compilation et validation des nouvelles versions
# avec vérification complète et historique
...
PROJECT_DIR="$(pwd)"
LOG_FILE="validation_automation.log"
...
check_prerequisites() {
    ...
    if ! pkg-config --exists x11; then
        ...
        sudo apt-get update && sudo apt-get install -y libx11-dev libgl1-mesa-dev libjson-c-dev
    fi
    
    # Vérifier les fichiers sources
    if [[ ! -f "test_validation_complete_v3.c" ]]; then
        error "Fichier test_validation_complete_v3.c manquant"
        return 1
    fi
...
run_validation() {
    ...
    VALIDATION_DIR="validation_v3_${TIMESTAMP}"
    
    # Lancer le test
    if ./test_validation_v3; then
...
analyze_results() {
    ...
    LATEST_DIR=$(ls -td validation_v3_* 2>/dev/null | head -1)
    ...
    if [[ -f "$LATEST_DIR/metrics_validation_v3_"*.json ]]; then
        METRICS_FILE=$(ls "$LATEST_DIR/metrics_validation_v3_"*.json | head -1)
    ...
    # Vérifier le rapport
    if [[ -f "$LATEST_DIR/rapport_validation_v3_"*.md ]]; then
        REPORT_FILE=$(ls "$LATEST_DIR/rapport_validation_v3_"*.md | head -1)
    ...
    # Vérifier les logs
    if [[ -f "$LATEST_DIR/logs_validation_v3_"*.log ]]; then
        LOG_FILE_VALIDATION=$(ls "$LATEST_DIR/logs_validation_v3_"*.log | head -1)
...
validate_integrity() {
    ...
    REQUIRED_FILES=("logs_validation_v3_" "metrics_validation_v3_" "rapport_validation_v3_")
        ...
        if compile_version; then
            if run_validation; then
                ...
                if [[ -n "$LATEST_DIR" ]]; then
                    ...
                    echo "📄 Logs: $LATEST_DIR/logs_validation_v3_*.log"
                    echo "📊 Métriques: $LATEST_DIR/metrics_validation_v3_*.json"
                    echo "📋 Rapport: $LATEST_DIR/rapport_validation_v3_*.md"
                fi
...
case "${1:-}" in
    "clean")
        ...
        rm -f validation_automation.log validation_history.json
